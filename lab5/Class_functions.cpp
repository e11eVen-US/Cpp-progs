...
{
    if ((eq_eq.club != eq_eq2.club) || (eq_eq.city != eq_eq2.city) || (eq_eq.coach != eq_eq2.coach)
        || (eq_eq.date != eq_eq2.date) || (eq_eq.budget != eq_eq2.budget) || (eq_eq.points != eq_eq2.points)
            || (eq_eq.place != eq_eq2.place)) return false;
    return true;
}

std::ostream& operator<<(std::ostream& stream, participant& test)
{
    ws; stream << test.club; ws;
    stream << test.city; ws;
    stream << test.coach; ws;
    stream << test.date; ws;
    stream << test.budget; ws;
    stream << test.points; ws;
    stream << test.place << '\n';
...
participant::participant(std::string a1, std::string a2, std::string a3, std::string a4, int a5, int a6, short a7)
{
    club = a1;
    city = a2;
    coach = a3;
